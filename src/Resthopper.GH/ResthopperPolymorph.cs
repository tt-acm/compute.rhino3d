...
namespace Resthopper.GH
{
    public class ResthopperPolymorph : GH_Component, IGH_VariableParameterComponent
    {
        ...
        protected override async void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (this.lastPointer == pointer)
            {
                ...
                for (int i = 1; i < this.Params.Input.Count; i++)
                {
                    ...
                    if (param.SourceCount < 1) { continue; }
                    string name = io.InputNames[i-1];
        ...
        private static async Task<Schema> SendRequest(Schema schema)
        {
            ...
            Schema result = JsonConvert.DeserializeObject<Schema>(response, settings);
            return result;
        }
